...
renderLanding();
startListeners();
let _authListenersUid = null; // guard: start auth listeners only once per uid
fbAuth.onAuthStateChanged(async (user)=>{
  if(user){
    const prof=await loadProfile(user.uid);
    // startAuthListeners fires on page load AND on token refresh (~every 1 hour).
    // Guard on uid so we never stack duplicate listeners.
    if(_authListenersUid !== user.uid){ _authListenersUid=user.uid; startAuthListeners(user.uid); }
