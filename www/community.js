...
function signInGoogle(){
  ...
  // Android/iOS Chrome opens a Custom Tab for popups but never posts the result back.
  // iPadOS 13+ sends a desktop Mac UA by default — detect it via maxTouchPoints.
  // Use redirect directly on all mobile/tablet devices to avoid silent auth failure.
  const isMobile=/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)||
                 (/Macintosh/i.test(navigator.userAgent)&&navigator.maxTouchPoints>1);
  if(isMobile){
    fbAuth.signInWithRedirect(provider).catch(e=>toast("Google sign-in failed: "+(e.code||e.message)));
    return;
  }
...
fbAuth.getRedirectResult().then(result=>{ if(result&&result.user) console.log("Redirect sign-in OK:",result.user.email); }).catch(e=>{ if(e.code==="auth/unauthorized-domain") toast("Login blocked: domain not authorised in Firebase. ("+location.hostname+")"); else if(e.code==="auth/web-storage-unsupported") toast("Sign-in blocked: your browser restricts storage. Try disabling private/incognito mode or allow cookies for this site."); else if(e.code&&e.code!=="auth/credential-already-in-use") toast("Google sign-in failed: "+(e.code||e.message)); });
...
async function finishOnboard(){
  ...
  try{
    ...
    // onAuthStateChanged ran before the profile existed so these were skipped — init now.
    listenForIncomingCalls(); initPushNotifications(); handleLoginSecurity(uid); initPresence(uid); E2EE.init(uid); checkLoginReward(uid); initCallPanel();
    ...
    $('page').innerHTML=`<div class="disc-v2-wrap">
      ...
      <div class="disc-lower">
        ...
        <div>
          ...
          ${appDownloadBanner()}${pinnedContestBanner()}${discoverComposer()}
...
function appDownloadBanner(){
  return `<div class="disc-app-banner">
    <div class="disc-app-banner-icon">💍</div>
    <div class="disc-app-banner-copy">
      <div class="disc-app-banner-title">VowCraft</div>
      <div class="disc-app-banner-sub">AI wedding vows — get the Android app</div>
    </div>
    <a class="disc-app-banner-cta" href="https://vowcraft-app.vercel.app/android" target="_blank" rel="noopener noreferrer">Get App</a>
  </div>`;
}
...
function logout(){ fbAuth.signOut(); /* onAuthStateChanged handles UI reset — no reload needed */ }
